...
class BaseSDK{
    ...
    private $host   = 'http://api.tuisong.baidu.com/rest/3.0/';
    ...
    const ERR_CLIENT        = 'Exception';
    const ERR_NET           = 'Exception';
    const ERR_SERVER        = 'Exception';

    private $log_output     = 'stdout';

    private $sign_expires   = 0;

    private $default_devicetype = 3;

    // 0  开发状态, rd开发时使用, 最详细 log. 发布后被移除
    // 1  开发者开发时状态,隐藏rd开发时的细碎信息
    // 2  开发者线上使用, 只有fault和warn
    // 3  开发者线上使用, 只有fault
    // 4  静默.
    private $log_level       = 4;
    ...
    function __construct($curlOpts = array()) {
        ...
        $this->log = new PushSimpleLog($this->log_output, $this->log_level );
        
        try {
            $this -> http = new HttpRequest($this->host, $curlOpts, $this->log);
        ...
        $this -> signExpires = intval($this->sign_expires);
    ...
    public function newApiQueryTpl() {
        ...
        if ($this -> deviceType === null && $this->default_devicetype !== null) {
            $this->setDeviceType($this->default_devicetype);
        }
